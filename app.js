    ...
    if (team == "blue") {
        for (var i = 0; i < blueTeam[room].length; i++) {
            if (blueTeam[room][i].name == name) {
                var temp1 = blueTeam[room].slice(0, i);
                var temp2 = blueTeam[room].slice(i + 1, blueTeam[room].length);
        ...
        for (var i = 0; i < redTeam[room].length; i++) {
            if (redTeam[room][i].name == name) {
                var temp1 = redTeam[room].slice(0, i);
                var temp2 = redTeam[room].slice(i + 1, redTeam[room].length);
                var temp3 = temp1.concat(temp2);
                redTeam[room] = temp3;
            }
        }
...
var nextQuestion = function(socket) {
    ...
    client.get("questions", function(err, reply) {
        ...
        var pytanie = allQuestions.questions[rand].question;

        var ansa = allQuestions.questions[rand].a;
        var ansb = allQuestions.questions[rand].b;
...
var pustyPokoj = function(pokoj) {
    if (io.sockets.clients(pokoj).length < 1 && pokoj > 1) {
        for (var i = 2; i < rooms.length; i++) {
            if (i == pokoj) {
                var temp1 = rooms.slice(0, i);
                var temp2 = rooms.slice(i + 1, rooms.length);
                var temp3 = temp1.concat(temp2);
                rooms = temp3;
            }
        }
